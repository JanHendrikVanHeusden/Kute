...
import nl.kute.test.java.printable.JavaClassToTestPrintableI
...
class PrintableITest {
    ...
    fun `test that toString with Mock does not break`() {
        val testMock: ClassToPrint = mock(arrayOf(PrintableI::class)) {
            on { num } doReturn 35
    ...
    fun `test with Kotlin subclass of Java class`() {
        val kotlinSubClass = KotlinClassToTestPrintableI("my str", 35, "this is another", names)
        assertThat(JavaClassToTestPrintableI::class.java.isAssignableFrom(kotlinSubClass.javaClass))
        assertThat(kotlinSubClass.toString()).isEqualTo("KotlinClassToTestPrintableI(str=my str, num=35, anotherStr=this is another, names=${names.contentDeepToString()})")
        assertThat(kotlinSubClass.asString()).isEqualTo("KotlinClassToTestPrintableI(str=my str, num=35, anotherStr=this is another, names=${names.contentDeepToString()})")
    }
    ...
    @Suppress("unused")
    open private class ClassToPrint(val str: String, open var num: Int, private val privateToPrint: Any?): PrintableI {
    ...
    private class KotlinClassToTestPrintableI(str: String, num: Int, val anotherStr: String, val names: Array<String>) :
        JavaClassToTestPrintableI(str, num, names) {
