@file:Suppress("KDocMissingDocumentation", "DuplicatedCode")
...
import nl.kute.performance.PerformanceFewProps.Companion.callCountPerMethodPerIteration
    ...
    fun setUpIteration() {
        ...
        tasks.toMutableList().shuffled().forEach { task ->
            repeat(callCountPerMethodPerIteration) {
                charCount.addAndGet(task(testObjectsFewProps[Random.nextInt(0, testObjectCount)]).length.toLong())
            }
        }
