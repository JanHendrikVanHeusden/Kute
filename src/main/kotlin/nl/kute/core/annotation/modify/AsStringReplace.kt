 ...
 * in the return value of [nl.kute.core.asString], but with its value replaced. This can be done either by means
...
@Inherited
@MustBeDocumented
annotation class AsStringReplace(
    /**
     * The expression to replace, either as regular expression or as a literal.
     *  * If [isRegexpPattern]:
    ...
    /**
     * * If `true` (default), [pattern] will be considered as a regular-expression pattern
     * * If `false`, [pattern] and [replacement] will be considered as literals using [String.replace];
     */
    val isRegexpPattern: Boolean = true
)
