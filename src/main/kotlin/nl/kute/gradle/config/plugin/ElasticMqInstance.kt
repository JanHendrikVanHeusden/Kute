...
internal class ElasticMqInstance(private val project: Project, val config: ServerInstanceConfiguration) {
    ...
    fun start() {
        ...
        sqsRestServer = SQSRestServerBuilder
            ...
            .withServerAddress(
                NodeAddress(config.protocol, config.host, config.port.toInt(), config.contextPath)
            )
            .start()
